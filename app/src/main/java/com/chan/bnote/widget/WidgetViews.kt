 ...
 *
 * 두 위젯의 레이아웃은 widget_root / widget_title / widget_text / widget_label / widget_message
 * 다섯 개의 id를 똑같이 갖고 있어서, 여기 함수들은 어느 위젯이든 그대로 쓸 수 있다.
 */
object WidgetViews {
	...
	fun applyTheme(context: Context, views: RemoteViews, theme: WidgetTheme) {
	...
	fun errorViews(context: Context, appWidgetId: Int): RemoteViews {
		...
		applyTheme(context, views, WidgetSettings.getTheme(context, appWidgetId))
